...
var (
	unknownCoder defaultCoder = defaultCoder{1, http.StatusInternalServerError, "An internal server error occurred", "http://imooc/goshop/pkg/errors/README.md"}
)
...
func Register(coder Coder) {
	if coder.Code() == 0 {
		panic("code `0` is reserved by `imooc/goshop/pkg/errors` as unknownCode error code")
	}
...
func MustRegister(coder Coder) {
	if coder.Code() == 0 {
		panic("code '0' is reserved by 'imooc/goshop/pkg/errors' as ErrUnknown error code")
	}

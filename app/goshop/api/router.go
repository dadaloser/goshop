...
func initRouter(ctx context.Context, g *restserver.Server, cfg *config.Config, clients ...*storage.Client) error {
	...
	)
	{
		...
		uGroup.GET("device_blacklist/self", jwtAuth.AuthFunc(), authz.RequirePrincipalTypes(authz.PrincipalCustomer), authz.RequirePermission(authz.PermissionUserDeviceBlacklistReadSelf), uController.ListOwnDeviceBlacklist)
		uGroup.POST("device_blacklist/self", jwtAuth.AuthFunc(), authz.RequirePrincipalTypes(authz.PrincipalCustomer), authz.RequirePermission(authz.PermissionUserDeviceBlacklistWriteSelf), uController.AddOwnDeviceBlacklist)
		uGroup.DELETE("device_blacklist/self/:device_id", jwtAuth.AuthFunc(), authz.RequirePrincipalTypes(authz.PrincipalCustomer), authz.RequirePermission(authz.PermissionUserDeviceBlacklistWriteSelf), uController.DeleteOwnDeviceBlacklist)

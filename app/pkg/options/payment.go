...
type PaymentOptions struct {
	...
	WorkerBatchTimeout time.Duration `json:"worker-batch-timeout" mapstructure:"worker-batch-timeout"`
...
func NewPaymentOptions() *PaymentOptions {
	return &PaymentOptions{Provider: "mock", CheckoutBaseURL: "https://payments.example.invalid/checkout", CallbackMaxSkew: 5 * time.Minute, RequestTimeout: 10 * time.Second, WorkerInterval: 5 * time.Second, WorkerBatchTimeout: 2 * time.Minute, WorkerBatchSize: 20, MaxAttempts: 8}
}
func (o *PaymentOptions) Validate() []error {
	...
	if o.RequestTimeout <= 0 || o.WorkerInterval <= 0 || o.WorkerBatchTimeout <= 0 || o.WorkerBatchSize <= 0 || o.MaxAttempts <= 0 {
...
func (o *PaymentOptions) AddFlags(fs *pflag.FlagSet) {
	...
	fs.DurationVar(&o.WorkerBatchTimeout, "payment.worker-batch-timeout", o.WorkerBatchTimeout, "maximum duration of one payment worker batch")
